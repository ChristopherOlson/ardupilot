...
void Copter::heli_acro_run()
{
    ...
    if (!motors->has_flybar()){
        ...
        if (g.acro_trainer == ACRO_TRAINER_DISABLED) {
            if (ap.land_complete) {
                heli_acro_virtual_flybar(target_roll, target_pitch, target_yaw, 3.0f, 3.0f);
            } else {
                heli_acro_virtual_flybar(target_roll, target_pitch, target_yaw, g.acro_balance_pitch, g.acro_balance_roll);
            }
        }
...
void Copter::heli_acro_virtual_flybar( float &roll_out, float &pitch_out, float &yaw_out, float pitch_leak, float roll_leak)
{
