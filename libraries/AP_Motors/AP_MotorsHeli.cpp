...
bool AP_MotorsHeli::parameter_check(bool display_msg) const
{
    ...
    if (_rsc_mode <= (int8_t)ROTOR_CONTROL_MODE_DISABLED || _rsc_mode > (int8_t)ROTOR_CONTROL_MODE_CLOSED_LOOP_POWER_OUTPUT) {
        if (display_msg) {
            GCS_MAVLINK::send_statustext_all(MAV_SEVERITY_CRITICAL, "PreArm: Throttle mode invalid");
        }
        return false;
    }
    ...
    if (_rsc_runup_time <= _rsc_ramp_time){
        if (display_msg) {
            GCS_MAVLINK::send_statustext_all(MAV_SEVERITY_CRITICAL, "PreArm: Runup time set too short");
        }
        return false;
    }
    ...
    if ( _rsc_idle_output >=  _rsc_critical){
        if (display_msg) {
            GCS_MAVLINK::send_statustext_all(MAV_SEVERITY_CRITICAL, "PreArm: Engine idle speed too high");
        }
        return false;
    }
